...
class ClaudeService {
  ...
  /// Server-side proxy (Supabase Edge Function). Preferred path: the
  /// Anthropic key lives in Supabase secrets instead of the app bundle.
...
''';
  ...
  Future<List<TripStop>> generateItinerary({
    ...
    // Preferred path: server-side proxy. Falls back to the legacy direct
    // call below so the app keeps working until the Edge Function is
    // deployed. Once it's deployed and the Anthropic key has been rotated
    // out of .env, the fallback is dead code and can be removed.
    try {
      ...
      // Covers FunctionException (404 = not deployed yet, 5xx = proxy
      // error) and network/timeout failures. During the migration window
      // the direct path below still has the key available.
      if (kDebugMode) {
        debugPrint('[Claude] edge function unavailable ($e) — '
  ...
  // ─── Legacy direct call (remove after Edge Function cutover) ─────────────

  Future<List<TripStop>> _generateDirect({
    required String destination,
    required String date,
    required String startTime,
    required String budget,
